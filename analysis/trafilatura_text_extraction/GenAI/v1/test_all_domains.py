...
def print_summary(results: Dict):
    ...
    categories = {
        ...
        'Special Case': ['FD'],
        'DPG Media Sites (Enhanced Selenium)': ['AD', 'ED', 'BD', 'Gelderlander', 'Destentor'],
    }
...
def run_comprehensive_test():
    ...
    scrapers = [
        ...
        DPGMediaScraper(headless=True, wait_time=6),  # Longer wait for problematic sites
    ]
...
def run_quick_test():
    ...
    for name, result in results.items():
        if result.success:
            print(f"✓ {name:10} - {len(result.article.content):5} chars - {result.time_taken:4.1f}s - {result.article.method}")
        else:
...
def test_fast_sites_only():
    ...
    fast_urls = {k: v for k, v in TEST_URLS.items() if k in ['NOS', 'Volkskrant', 'NRC', 'Trouw', 'Parool']}
...
def test_dpg_sites_only():
    ...
    dpg_urls = {k: v for k, v in TEST_URLS.items() if k in ['AD', 'ED', 'BD', 'Gelderlander', 'Destentor']}
