            ...
            if len(sentence) >= min_context_chars * 2:
                ...
                if entity_pos_in_text != -1:
                    ...
                    start = max(0, entity_pos_in_text - min_context_chars)
                        ...
                        len(text), entity_pos_in_text + len(entity) + min_context_chars
                    )
...
def entity_sentiments(
    ...
    articles_to_process = add_features.filter(
        ~pl.col("link").is_in(
            historic_entity_sentiments["link"]
    ...
    combined_entity_sentiments = pl.concat(
        [historic_entity_sentiments, processed_entity_sentiments]
    )
...
def sentiments(entity_sentiments: DataFrame) -> DataFrame:
    ...
    exploded = (
        ...
        .with_columns(decoded=pl.col("value").str.json_decode(schema))
