...
def detailed_failure_message(context: dg.RunFailureSensorContext) -> str:
    ...
    if step_failure_events:
        ...
        for event in step_failure_events:
            if event.event_specific_data and hasattr(event.event_specific_data, 'error'):
                ...
                error_messages.append(
                    f"Step {event.step_key}: {error_msg}"
                )
        if error_messages:
            message_parts.append("*Step Failures:*\n" + "\n".join(error_messages[:2]))  # Max 2 steps
