...
class SeleniumScraper(ArticleScraper):
        ...
        for script in scripts:
            try:
                ...
                if isinstance(data, list):
                    data = next(
                        (d for d in data if d.get("@type") == "NewsArticle"), None
                    )
    ...
    def _extract_html(self, url: str, soup: BeautifulSoup) -> Optional[Article]:
        ...
        for tag in article_elem.find_all(
        ...
        return Article(url=url, title=title, content=content, method="selenium_html")
...
class DutchNewsScraperService:
    ...
    def scrape(self, url: str) -> ScrapeResult:
        ...
        try:
            ...
            if article:
                return ScrapeResult(
                    success=True, article=article, time_taken=time.time() - start_time
                )
            else:
                return ScrapeResult(
                    success=False,
            ...
            return ScrapeResult(
                success=False, error=str(e), time_taken=time.time() - start_time
            )

    def scrape_batch(
        self, urls: Dict[str, str], progress_callback=None
