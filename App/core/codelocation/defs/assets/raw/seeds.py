...
def news_agencies() -> DataFrame:
    ...
    df = pl.read_csv(
        DATA_DIR / "seeds/news_agencies.csv", try_parse_dates=True, separator=";"
    )
    return df.with_columns(
        ...
        left_right_label=(
            ...
            .then(pl.lit("right"))
            .when(
                (pl.col("left_right") >= centre_right) & (pl.col("left_right") < right)
            )
            .then(pl.lit("centre right"))
            .when(
                (pl.col("left_right") < centre_right)
                & (pl.col("left_right") > centre_left)
            )
            .then(pl.lit("centre"))
...
def agency_owners() -> DataFrame:
    ...
    return pl.read_csv(
        DATA_DIR / "seeds/agency_owners.csv", try_parse_dates=True, separator=";"
    )
